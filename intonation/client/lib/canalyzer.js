...
//analyzer.js
//Use web audio api to produce analyze data

(function() {

  //All note handle by analyzer in Latin representation
  ...
  var computeHzFromSteps = function(steps) {
    var f0 = 440;
    var fn = f0 * Math.pow(Math.pow(2, 1.0 / 12), steps);
    return fn;
  };

  var Analyzer = function(controllers, precision, highFrequency) {
    //max frequency to analyze
  ...
  Analyzer.prototype = {
    ...
    install: function(onReady, onError) {
      ...
      OnlineTuner.getUserMedia({
        ...
        ms2 = stream;
        window.source = self.audioContext.createMediaStreamSource(stream);
    ...
    getInfo: function() {
      ...
      var step = computeStep(frequency); //true steps off
      var note = computeNote(step); //closest note
      var octave = computeOctave(step); //octave of closest note

      var steps_from_a4 = note_order[note] - 9 + (octave - 4) * 12;
      var closest_pitch = computeHzFromSteps(steps_from_a4);

      //want note name to half steps away function
      return {
        frequency: frequency,
        step: step,
        error: this.getError(frequency),
        stepError: this.getStepError(frequency),
        pitch_error: frequency - closest_pitch,
        note: note,
        octave: octave
      };
    },
